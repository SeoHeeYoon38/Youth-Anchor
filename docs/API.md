...
| 온통청년 청년정책 API | 별도 수동 승인과 운영 의존성을 피하기 위해 사용하지 않습니다. 지원사업은 공공데이터포털의 중앙부처복지서비스 API만 동기화합니다. |
| 쉼터 실시간 잔여석 | 잔여석을 제공하는 공개 API가 없어 저장하거나 화면에 노출하지 않습니다. 출발 전 시설 또는 1388에 확인하도록 안내합니다. |
...
`OPENAI_API_KEY`가 설정되면 서버가 OpenAI Responses API로 가온의 답변을 생성합니다. 브라우저에는 키를 전달하지 않으며 응답 저장 옵션도 끕니다. 외부 API가 일시적으로 실패하거나 사용 한도에 도달해도 112·1388·쉼터·지원사업으로 연결하는 최소 안전 안내는 서버에서 계속 제공합니다.
