...
| 화면 | 데이터 출처 |
| --- | --- |
| 대피처 (`/shelters`) | 성평등가족부_청소년쉼터 오픈API |
| 자립 지원 (`/support`) | 한국사회보장정보원_중앙부처복지서비스 오픈API |
| 익명 상담·긴급 SOS | 자체 DB + WebSocket |
...
키를 넣기 전에는 이름에 `샘플`이 붙은 쉼터 4건만 표시되며, 실제 데이터가 수집되면 자동으로 교체됩니다. 자세한 수집 규칙과 연동하지 못한 API 목록은 [API 문서](docs/API.md)에 정리되어 있습니다.
