...
export default function SheltersPage({ shelters, position, locationMessage, locateMe, locating, onSelectShelter }) {
  ...
  const visibleShelters = useMemo(() => filterShelters(shelters, filter), [filter, shelters])
    ...
    <>
      <div className="view shelter-view page-enter">
        ...
        <div className="map-controls">
          ...
          <button className={filterOpen || filterActive ? 'filter-button active' : 'filter-button'} onClick={() => setFilterOpen((open) => !open)} aria-expanded={filterOpen}>
            <SlidersHorizontal size={17} /> 조건{filterActive ? ` · ${describeFilter(filter)}` : ''}
          </button>
        ...
        {filterOpen && (
          <div className="filter-panel page-enter">
            ...
            <div className="filter-footer">
              ...
              {filterActive && (
                <button className="filter-reset" onClick={() => setFilter(DEFAULT_SHELTER_FILTER)}>
        ...
        {visibleShelters.length === 0 && (
          <div className="empty-state">
            ...
            <p>{filterActive ? `'${describeFilter(filter)}' 조건을 바꾸거나 초기화해 보세요.` : '잠시 후 다시 확인해 주세요.'}</p>
            {filterActive && (
              <button className="filter-reset" onClick={() => setFilter(DEFAULT_SHELTER_FILTER)}>
                <RotateCcw size={14} /> 조건 초기화
              </button>
